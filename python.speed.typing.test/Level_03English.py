...
class level_03:
    def __init__(GUI3):
        # Window Maker
        GUI3.window = Tk()
        GUI3.window.title("Speed Typing Game -- level 03 --")
        ...
        # Title
        Title = Label(GUI3.window, text='Congratulations!, and Welcome to Level 03', bg="#286ED3", font=("Comic Sans MS", 25, "bold"), width=35, fg="white")
        Title.place(x=80, y=80)

        # paragraphs To Be Written
        with open("paragraphs_English.txt", "r") as file: GUI3.paragraphs = file.read().split('\n\n\n')
        ...
        # paragraphs Label
        paragraph_label = Label(GUI3.window, text="Paragraphs", font=("Casteller", 28, "bold"), bg="#286ED3", fg="white")
        paragraph_label.place(x=80, y=150)

        # paragraphs Counter
        paragraph_counter = Label(GUI3.window, text=i, font=("Casteller", 28, "bold"), bg="#286ED3", fg="white")
        paragraph_counter.place(x=170, y=220)

        # Time Label
        timer_label = Label(GUI3.window, text="Timer", font=("Casteller", 28, "bold"), bg="#286ED3", fg="white")
        timer_label.place(x=720, y=150)

        # Timer Counter
        timer_counter = Label(GUI3.window, text=time_left, font=("Casteller", 28, "bold"), bg="#286ED3", fg="white")
        timer_counter.place(x=755, y=220)
        ...
        # Instructions
        instruction = Label(GUI3.window, text = "Type the paragraph & press PLUS to start or ENTER for a new line", font=("Chiller", 20, "bold"), bg="#286ED3", fg="white")
        ...
        def retry():
            GUI3.window.destroy()
            call(["python", "Level_03English.py"])
        ...
        # Next Level Button
        next_level_button = Button(GUI3.window, text="NEXT", command=next_level, image=next_image, compound=LEFT, bg="White", font=("Comic Sans MS", 15, "bold"), width=100, fg="Black")
        
        # Retry Image
        retry_image = PhotoImage(file="Retry.png")
        # Retry Button
        retry_button = Button(GUI3.window, text="Retry", command=retry, image=retry_image, compound=RIGHT, bg="White", font=("Comic Sans MS", 15, "bold"), width=100, fg="Black")
        
        # Home Image
        home_image = PhotoImage(file="Home.png")
        # Home Button
        home_button = Button(GUI3.window, text="Home", command=home, image=home_image, compound=LEFT, bg="White", font=("Comic Sans MS", 15, "bold"), width=100, fg="Black")
        ...
        def timer():
            ...
            if time_left > 0:
                ...
                new_title = Label(GUI3.window, text="TIME IS UP", bg="#286ED3", font=("Comic Sans MS", 50, "bold"), fg="white")
                new_title.place(x=240, y=280)

                result = correct_paragraph - wrong_paragraph
                instruction.config(text=f"Correct paragraphs: {correct_paragraph}\n Wrong paragraphs: {wrong_paragraph}\n Final Score: {result}")
