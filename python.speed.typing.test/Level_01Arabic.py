...
class level_01:
    ...
    def __init__(GUI1):
        ...
        # Window Maker
        GUI1.window = Tk()
        GUI1.window.title("<-- لعبة سرعة الكتابة <-- المستوى ١")
        ...
        # Title
        GUI1.Title = Label(GUI1.window, text = "--- ٠١ المستوى في بكم مرحبا ---", bg = "#286ED3", font = ("Comic Sans MS", 30, "bold"), width = 35, fg = "white")
        GUI1.Title.place(x=20, y=80)

        # Words To Be Written
        GUI1.words = open("Words_Arabic.txt", encoding = "utf-8").read().split("\n")
        ...
        # Words Label
        GUI1.word_label = Label(GUI1.window, text = "كلمات", font = ("Casteller", 30, "bold"), bg = "#286ED3", fg = "white")
        ...
        GUI1.word_counter.place(x = 110, y = 220)

        # Time Label
        GUI1.timer_label = Label(GUI1.window, text = "الوقت", font = ("Casteller", 30, "bold"), bg = "#286ED3", fg = "white")
        GUI1.timer_label.place(x = 730, y = 150)

        # Timer Counter
        GUI1.timer_counter = Label(GUI1.window, text = GUI1.time_left, font = ("Casteller", 28, "bold"), bg = "#286ED3", fg = "white")
        GUI1.timer_counter.place(x = 750, y = 220)
        ...
        # Instructions
        GUI1.instruction = Label(GUI1.window, text = "ENTER على واضغط الكلمة اكتب ", font = ("Chiller", 28, "bold"), bg = "#286ED3", fg = "white")
        ...
        # Next Level Button
        GUI1.next_level_button = Button(GUI1.window, text="التالى", command=next_level, image=GUI1.next_image, compound=RIGHT, bg="White", font=("Comic Sans MS", 15, "bold"), width=100, fg="Black")
        
        # Retry Image
        GUI1.retry_image = PhotoImage(file="Retry.png")
        # Retry Button
        GUI1.retry_button = Button(GUI1.window, text="اعادة", command=retry, image=GUI1.retry_image, compound=RIGHT, bg="White", font=("Comic Sans MS", 15, "bold"), width=100, fg="Black")
        
        # Home Image
        home_image = PhotoImage(file="Home.png")
        # Home Button
        home_button = Button(GUI1.window, text="رجوع", command=home, image=home_image, compound=LEFT, bg="White", font=("Comic Sans MS", 15, "bold"), width=100, fg="Black")
    ...
    def timer(GUI1):
            ...
            if GUI1.time_left > 0:
                ...
                new_title = Label(GUI1.window, text = "الوقت انتهى لقد", bg = "RoyalBlue3", font = ("Comic Sans MS", 50, "bold"), fg = "white")
                new_title.place(x = 240, y = 255)
                
                result = GUI1.correct_word - GUI1.wrong_word
                GUI1.instruction.config(text = f"{GUI1.correct_word} :صحيحة كلمات \n {GUI1.wrong_word} :خاطئة كلمات\n {result} :النهائية النتيجة")
